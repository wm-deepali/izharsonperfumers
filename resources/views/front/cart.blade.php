...
@section('content')
    ...
    2
            ...
            <div class="ui_kit_button payment_widget_btn">
              @if(Auth::guard('customer')->check())
                <button type="button" class="btn btn-thm btn-block checkout-nav-btn" data-href="{{ route('checkout') }}">
                  Proceed to checkout
                </button>
              @else
                <button type="button" class="btn btn-thm btn-block checkout-nav-btn"
                  data-href="{{ route('customer.login', ['redirect' => route('checkout')]) }}">
                  Login to Checkout
                </button>
              @endif
            </div>
  ...
  <script>
    document.addEventListener("click", function (e) {
      const btn = e.target.closest(".checkout-nav-btn");
      if (btn) {
