...
@section('content')
    ...
    <section class="shop-single-content pb80 pt0 ovh">
                    ...
                    <div class="column">
                        <div class="shop_single_natabmenu">
                            <div class="d-flex flex-column-reverse flex-sm-row align-items-start">
                                ...
                                <!-- Thumbnails -->
                                <div class="nav flex-row mt-2 mt-md-0 flex-md-column nav-pills me-0 me-md-3 gap-2"
        ...
        : 'No reviews' }} )
                            ...
                            <div class="mb-0">
                                <ul class="cart_btn_widget shop_single3_style db-767 d-flex mb-0">
                                    ...
                                    <li class="me-0 me-sm-3 mb-3"><a href="#"
                                            class="btn btn-white bdr_thm bdrs60 buy-now-btn"
                                            data-product="{{ $product->id }}">
                                            Buy Now
                                        </a></li>
                                </ul>
                                <ul class="shop_single_wishlist_area  d-flex align-items-center mb-3"
                                                        ...
                                                        <div class="product_single_content mb30">
                                                            <div class="mbp_pagination_comments">
                                                                ...
                                                                @forelse($product->product_review as $review)
                                                                    ...
                                                                    : asset('front/images/blog/reviewer2.png') }}" class="mr-3" alt="reviewer">
                                                                                                                            </div>
                                                        ...
                                                        @if(auth()->guard('customer')->check() && $canReview)
                                                            ...
                                                            <div class="bsp_reveiw_wrt mb-0">
                                                                <form class="comments_form"
                                                                    action="{{ route('customer.review.submit') }}"
                                                                    method="POST">
                                                                    @csrf
        ...
        : 'No reviews' }} )
                            ...
                            <div class="d-block d-sm-flex align-items-start">
                                <div class="nav nav-pills" role="tablist">
                                    ...
                                    @foreach($product->product_options as $index => $option)
                                        ...
                                        <button class="nav-link me-2 {{ $index == 0 ? 'active' : '' }}" data-bs-toggle="pill"
                                            data-bs-target="#pack{{ $index }}" type="button" role="tab"
                                            data-price="{{ $option->price }}" data-mrp="{{ $option->mrp }}"
                        ...
                        <hr>
                        <ul
                            class="cart_btn_widget shop_single3_style align-items-center d-flex justify-content-between mb-1">
                            <li class="list-inline-item me-3 mb-2">
                                <div class="cart_btn home_page_sidebar d-grid">
                                    <div class="quantity-block home_page_sidebar">
                                        <button class="quantity-arrow-minus2 shop_single_page_sidebar"><img
                                                src="{{ asset('front/images/icons/minus.svg') }}" alt=""></button>
                                        <input class="quantity-input quantity-num2 shop_single_page_sidebar" type="number"
                                            value="1" min="1">
                                        <button class="quantity-arrow-plus2 shop_single_page_sidebar"> <span
                                                class="flaticon-close"></span> </button>
                                    </div>
    ...
    <script>
        ...
        function applyCartButtonState(button) {
            ...
            button.dataset.addedLabel = button.dataset.addedLabel || 'Already in Cart';
            button.innerHTML = button.dataset.addedLabel;
        }
        ...
        // ---- ADD TO CART (delegated — covers main page buttons + carousel cards) ----
        document.addEventListener('click', function (e) {
            ...
            const optionId = button.dataset.option || selectedOption || null;
            const qtyInput = document.querySelector('.quantity-input');
            const quantity = qtyInput ? (qtyInput.value || 1) : 1;
            ...
            fetch("{{ route('cart.store') }}", {
                ...
                .then(data => {
                    ...
                    if (data.cart_count !== undefined) {
                        ...
                        // Both instances on this page (mobile + sidebar) point to the
                        // same product — mark the other one too, if it's not already.
                        document.querySelectorAll(`.add-to-cart-btn[data-product="${productId}"]`).forEach(other => {
                            if (other !== button && !other.classList.contains('in-cart')) {
                                if (!other.dataset.originalText) {
                                    other.dataset.originalText = other.innerHTML;
                                }
                                applyCartButtonState(other);
                            }
                        });
                    } else {
        ...
        // ---- BUY NOW (unchanged logic, still adds to cart first) ----
        document.querySelectorAll('.buy-now-btn').forEach(btn => {
            btn.addEventListener('click', function (e) {
                ...
                fetch("{{ route('cart.store') }}", {
                    ...
                    .catch(() => {
                        Swal.fire({ icon: 'error', title: 'Error', text: 'Unable to process Buy Now' });
                    });
            });
        });
