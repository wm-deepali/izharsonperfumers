                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                @if (isset($category->banner_image))
                                                                    <a href="javascript:void(0)" class="view-image" category_id="{{ $category->id }}">
                                                                        <img src="{{ URL::asset('storage/' . $category->banner_image) }}" height="50" width="50">
                                                                    </a>
                                                                @else
                                                                    NA
                                                                @endif
...
<script>
     $(document).on("click", ".show-category", function(event) {
            ...
            $.ajax({
                url: `{{ url('admin/manage-category/show-category/${id}') }}`,
                type: "get",
                dataType: "json",
    ...
    function updateStatus(id){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ URL::to('admin/manage-category/change-status/${id}') }}`,
                    type: "POST",
                    brandType: "json",
                    success: function(result) {
                        if (result.success) {
    ...
    $(document).ready(function(event) {
        ...
        $(document).on("click", "#add-category-btn", function(event) {
            ...
            $('#slug-err').html('');
            $('#image-err').html('');
            $('#banner_image-err').html('');
            $('#meta_title-err').html('');
            $('#twitter_cards-err').html('');
            $('#meta_keyword-err').html('');
            $('#canonical_tags-err').html('');
