                ...
                <div class="row breadcrumbs-top">
                    <div class="breadcrumb-wrapper col-xs-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Manage Car Model</li>
                        </ol>
                    </div>
                </div>
            </div>
        ...
        <div class="col-xl-12 col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">MANAGE - CAR-MODEL</h4>
                                ...
                                <div class="card-block card-dashboard">
                                    <div class="table-responsive">
                                        <table class="table table-striped table-bordered" id="example">
                                            ...
                                            <tbody>
                                                @if (isset($brandmodels) && count($brandmodels) > 0)
...
<script>
     function updateStatus(id){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ URL::to('admin/manage-brand-models/change-status/${id}') }}`,
                    type: "POST",
                    dataType: "json",
    ...
    function deleteConfirmation(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ URL::to('admin/manage-brand-models/${id}') }}`,
                    type: "DELETE",
                    dataType: "json",
                    success: function(result) {
                        if (result.success) {
    ...
    $(document).ready(function(event) {
        $(document).on("click", ".add-brand", function(event) {
            $.ajax({
                url: "{{ URL::to('admin/manage-brand-models/create') }}",
                type: "GET",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
        ...
        $(document).on("click", ".show-brand", function(event) {
            ...
            $.ajax({
                url: `{{ url('admin/manage-brand-models/${id}') }}`,
                type: "get",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
        ...
        $(document).on("click", ".add-brand-btn", function(event) {
            ...
            formData.append('brand_id', $('#brand_id').val());
             var checkvalue = [];
            $.each($("input[name='fueltype[]']:checked"), function(){            
                checkvalue.push($(this).val());
            });
            var cylindervalue = [];
            $.each($("input[name='cylinder[]']:checked"), function(){            
                cylindervalue.push($(this).val());
            });
            formData.append('fueltype', checkvalue);
            formData.append('cylinder', cylindervalue);
            formData.append('status', $('#status').val());
            formData.append('image', (typeof $('#image')[0].files[0] == 'undefined') ? '' : $('#image')[0].files[0]);
            $.ajax({
                url: "{{ URL::to('admin/manage-brand-models') }}",
                type: 'POST',
                processData: false,
                contentType: false,
                dataType: 'json',
        ...
        $(document).on("click", ".edit-brand", function(event) {
            let id = $(this).attr('brand_id');
            $.ajax({
                url: `{{ URL::to('admin/manage-brand-models/${id}/edit') }}`,
                type: "get",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
        ...
        $(document).on("click", "#update-brandmodel-btn", function(event) {
            $(this).attr('disabled', true);
            $('#name-err').html('');
            $('#url-err').html('');
            $('#image-err').html('');
            ...
            formData.append('cylinder', cylindervalue);
            formData.append('image', (typeof $('#image')[0].files[0] == 'undefined') ? '' : $('#image')[0].files[0]);
            let brand_id = $(this).attr('brandmodel_id');
            $.ajax({
                url: `{{ URL::to('admin/manage-brand-models/${brand_id}') }}`,
                type: 'POST',
                processData: false,
                contentType: false,
                dataType: 'json',
