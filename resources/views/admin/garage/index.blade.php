                ...
                <div class="card-header">
                    ...
                    <div class="heading-elements">
                        <ul class="list-inline mb-0">
                            <li><a href="javascript:void(0)" class="add-garage"><i class="fa fa-plus"></i> Add </a></li>
                                                    ...
                                                    @foreach ($datas as $data)
                                                        <tr>
                                                            ...
                                                            <td class="text-truncate">
                                                                <ul class="actions">
                                                                    <li><a href="javascript:void(0)" class="edit-garage" garage_id="{{ $data->id }}" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a></li>
...
<div id="garage-modal" class="modal fade" role="dialog">
</div>
...
<script>
    function deleteConfirmation(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ URL::to('admin/manage-garage/${id}') }}`,
                    type: "DELETE",
                    dataType: "json",
    ...
    $(document).ready(function() {
        ...
        $(document).on("click", ".add-garage", function(event) {
            $.ajax({
                url: "{{ URL::to('admin/manage-garage/create') }}",
                type: "GET",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
                        $("#garage-modal").html(result.html);
        ...
        $(document).on("click", "#add-garage-btn", function(event) {
            ...
            $.ajax({
                url: "{{ URL::to('admin/manage-garage') }}",
                type: 'POST',
                processData: false,
                contentType: false,
                dataType: 'json',
        ...
        $(document).on("click", ".edit-garage", function(event) {
            let id = $(this).attr('garage_id');
            $.ajax({
                url: `{{ URL::to('admin/manage-garage/${id}/edit') }}`,
                type: "get",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
        ...
        $(document).on("click", "#update-garage-btn", function(event) {
            for (instance in CKEDITOR.instances) {
                CKEDITOR.instances[instance].updateElement();
            }
            $(this).attr('disabled', true);
            ...
            formData.append('og_tags', $('#og_tags').val());
            formData.append('image', (typeof $('#image')[0].files[0] == 'undefined') ? '' : $('#image')[0].files[0]);
            let garage_id = $(this).attr('garage_id');
            $.ajax({
                url: `{{ URL::to('admin/manage-garage/${garage_id}') }}`,
                type: 'POST',
                processData: false,
                contentType: false,
                dataType: 'json',
