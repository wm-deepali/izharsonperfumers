    ...
    <div class="content-wrapper">
        <div class="content-header row">
            <div class="content-header-left col-md-6 col-xs-12 mb-2">
                <h3 class="content-header-title mb-0">SERVICE CATEGORIES</h3>
                <div class="row breadcrumbs-top">
                    <div class="breadcrumb-wrapper col-xs-12">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                            {{-- <li class="breadcrumb-item">Catalog</li> --}}
                            <li class="breadcrumb-item active">Manage Service Categories</li>
                        </ol>
                    </div>
                </div>
            </div>
        ...
        <div class="col-xl-12 col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">MANAGE - Service CATEGORIES</h4>
                                                ...
                                                @if (isset($categories) && count($categories) > 0)
                                                    @foreach ($categories as $category)
                                                        <tr>
                                                            ...
                                                            <td>{{ $category->name }}</td>
                                                            <td>{{ $category->other_service }}</td>
                                                            <td>{{ $category->value_added_service }}</td>
                                                            
                                                            <td>{{ $category->status=="active" ? "Active" : "De-Active" }}</td>
                                                            <td class="text-truncate">
                                                                <ul class="actions">
                                                                <li><a href="javascript:void(0)" class="show-category" category_id="{{ $category->id }}" title="View"><i class="fa fa-eye" aria-hidden="true"></i></a></li>
   ...
   function updateStatus(id){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ URL::to('admin/manage-service-category/change-status/${id}') }}`,
                    type: "POST",
                    brandType: "json",
    ...
    $(document).on("click", ".show-category", function(event) {
            let id = $(this).attr('category_id');
            $.ajax({
                url: `{{ url('admin/manage-service-category/show/${id}') }}`,
                type: "get",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
    ...
    function deleteConfirmation(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ URL::to('admin/manage-service-category/${id}') }}`,
                    type: "DELETE",
                    dataType: "json",
                    success: function(result) {
                        if (result.success) {
    ...
    $(document).ready(function(event) {
        $(document).on("click", "#add-category", function(event) {
            $.ajax({
                url: "{{ URL::to('admin/manage-service-category/create') }}",
                type: "GET",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
        ...
        $(document).on("click", "#add-category-btn", function(event) {
            ...
            $.ajax({
                url: "{{ URL::to('admin/manage-service-category') }}",
                type: 'POST',
                processData: false,
                contentType: false,
                dataType: 'json',
        ...
        $(document).on("click", ".edit-category", function(event) {
            let id = $(this).attr('category_id');
            $.ajax({
                url: `{{ URL::to('admin/manage-service-category/${id}/edit') }}`,
                type: "get",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
        ...
        $(document).on("click", "#update-category-btn", function(event) {
            ...
            formData.append('status', $('#status').val());
            formData.append('image', (typeof $('#image')[0].files[0] == 'undefined') ? '' : $('#image')[0].files[0]);
             formData.append('other_service', $('#other_service').prop('checked') == true ? "yes" : "no");
            formData.append('value_added_service', $('#value_added_service').prop('checked') == true ? "yes" : "no");
            let category_id = $(this).attr('category_id');
            $.ajax({
                url: `{{ URL::to('admin/manage-service-category/${category_id}') }}`,
                type: 'POST',
                processData: false,
                contentType: false,
                dataType: 'json',
