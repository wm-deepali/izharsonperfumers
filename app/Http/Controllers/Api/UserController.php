...
class UserController extends Controller
{
   ...
   if($validator->passes()){
    ...
    if($request->order_status!="Delivered" && $request->order_status!="Cancelled"){
        $orders = Order::latest()->where('customer_id',$customer->id)->where('order_status','!=','Delivered')->with('countries:id,name','states:id,name','citys:id,name')->where('order_status','!=','Cancelled')->with('order_details.product:id,name,image','order_details.category:id,name,image')->with('order_details')->get(['id','order_number','order_amount_with_shipping As order_amount','estimated_delivery_date','order_status','address','invoice_number','invoice_url','created_at as order_date_time','payment_status','coupon_code','order_amount as subtotal','discount_amount','shipping_type_price as shipping_price','total_gst_amount','order_amount_with_shipping','country','state','city','name','address','mobile_number','email','pincode']);
