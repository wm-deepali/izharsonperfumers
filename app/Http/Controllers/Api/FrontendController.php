...
foreach($cart_categories as $product) {
    if($product->category_id) $cart_category_ids[] = $product->category_id;
    if($product->subcategory_id) $cart_category_ids[] = $product->subcategory_id;
}
  ...
  public function filterproduct(Request $request)
{
  ...
  public function filterproductbck(Request $request){
   $brandmodel = $request->brandmodel_id;
    ...
    $no = 15;
    if($request->subcategory_id){
        $objs = Product::with('product_options')->where('status','active')->whereIn('subcategory_id',$request->subcategory_id)->with('product_option_images')->with(['product_options'=>function($opt) use($brandmodel,$price,$category,$brandid){
        //   $opt->where('mrp','>=',$price)->get();
    }])->paginate($no); 
    }else if($request->category_id){
         $objs = Product::with('product_options')->where('status','active')->whereIn('category_id',$request->category_id)->with('product_option_images')->with(['product_options'=>function($opt) use($brandmodel,$price,$category,$brandid){
        //   $opt->where('mrp','>=',$price)->get();
    }])->paginate($no);
    }else if ($request->min_price){
         $objs = Product::with('product_options')->where('status','active')->with('product_option_images')->with(['product_options'=>function($opt) use($brandmodel,$price,$category,$brandid,$maxprice){
           $opt->where('price','>=',$price)->where('price','<=',$maxprice)->get();
    }])->paginate($no);
    }else if($request->subcategory_id && $request->category_id){
        $objs = Product::with('product_options')->where('status','active')->whereIn('subcategory_id',$request->subcategory_id)->whereIn('category_id',$request->category_id)->with('product_option_images')->with(['product_options'=>function($opt) use($brandmodel,$price,$category,$brandid,$maxprice){
        //   $opt->where('mrp','>=',$price)->get();
    }])->paginate($no);
    }else if($request->subcategory_id && $request->min_price){
        $objs = Product::with('product_options')->where('status','active')->whereIn('subcategory_id',$request->subcategory_id)->with('product_option_images')->with(['product_options'=>function($opt) use($brandmodel,$price,$category,$brandid,$maxprice){
         $opt->where('price','>=',$price)->where('price','<=',$maxprice)->get();
    }])->paginate($no);
    }else if($request->min_price && $request->category_id){
        $objs = Product::with('product_options')->where('status','active')->whereIn('category_id',$request->category_id)->with('product_option_images')->with(['product_options'=>function($opt) use($brandmodel,$price,$category,$brandid,$maxprice){
          $opt->where('price','>=',$price)->where('price','<=',$maxprice)->get();
    }])->paginate($no); 
...
public function getCartserviceUnauthenticated(Request $request) {
    ...
    try {
        ...
        $cart_items = UnAuthCartServiceDetail::where('cart_id',$cart->id)->with(['services' => function ($query) {
        $query->select('id','name','name_ar','image');
    }])->with(['service_options' => function ($query1) {
...
public function getshippingtype(Request $request){
    ...
    if(isset($default_shipping_cost)){
        $shippingData[]=["id"=>$default_shipping_cost->id,
            'in_state_charge'=>0,
